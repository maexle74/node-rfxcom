...
function RfxCom(device, options) {
    ...
    self.TxQ.on("timeout", function (next, transmission) {
        if (self.options.debug) {
            console.log("[rfxcom] on " + self.device + " - Error   : Command message " + self.dumpHex([transmission.seqnbr]) +
        ...
        if (data.length != length) {
            ...
            if (typeof handler !== "undefined") {
                try {
                    ...
                    if (self.options.debug && e instanceof Error) {
                        console.log("[rfxcom] on " + self.device + " - " + "Packet type %s handler threw exception %s: %s",
                            self.dumpHex([packetType]), e.name, e.message);
                    }
                }
            } else {
                if (self.options.debug) {
...
RfxCom.prototype.close = function () {
    ...
    self.TxQ.end();
    if (self.options.debug) {
        console.log("[rfxcom] on " + self.device + " - Cleared command message queue");
    }
...
RfxCom.prototype.messageHandler = function(data) {
    ...
    if (self.acknowledge[seqnbr] != undefined && typeof self.acknowledge[seqnbr] == "function") {
        if (self.options.debug) {
            console.log("[rfxcom] on " + self.device + " - Response: Command message " +
                        self.dumpHex([seqnbr]) + ", " + responses[message]);
        }
...
RfxCom.prototype.statusHandler = function(data) {
    ...
    if (subtype === 0xFF) {         // Message not understood!
        ...
        if (self.options.debug) {
            console.log("[rfxcom] on " + self.device + " - Response: Command message " +
                self.dumpHex([seqnbr]) + ", command unknown or not supported by this device");
        }
        ...
        if (self.initialising) {
            ...
            self.TxQ.start();
            if (self.options.debug) {
                console.log("[rfxcom] on " + self.device + " - Started command message queue");
            }
        }
        ...
        if (copyrightText == "Copyright RFXCOM") {
            if (self.options.debug) {
                console.log("[rfxcom] on " + self.device + " - " + copyrightText);
            }
            self.initialising = false;
            self.TxQ.start();
            if (self.options.debug) {
 ...
 */
RfxCom.prototype.queueMessage = function(buffer, seqnbr, callback) {
    var self = this;

    if (self.connected) {
        if (self.options.debug) {
            console.log("[rfxcom] on " + self.device + " - " + "Queued  : %s", self.dumpHex(buffer));
        }
